...
def main(file):
    btmp = file
    objects = []
    radius = (512 / 16) * (1. - 0.7 * (btmp.cs - 5) / 5)
    ...
    class d_obj:
        ...
        def calculate_strain(self,prev, dtype):
            ...
            res = 0
            time_elapsed = (int(self.ho.time) - int(prev.ho.time)) / btmp.speed
    ...
    def calculate_difficulty(p_type, objects):
        strain_step = 400 * btmp.speed
        prev = None
        max_strain = 0
        decay_weight = 0.9
        highest_strains = []
        interval_end = strain_step
        for obj in btmp.objects:
            new = d_obj(obj, radius,prev)
            objects.append(new)
            while int(new.ho.time) > interval_end:
                highest_strains.append(max_strain)
                if prev is None: max_strain = 0
                else:
                    decay = math.pow(consts.decay_base[p_type],(interval_end - int(prev.ho.time)) / 1000.0)
                    max_strain = prev.strains[p_type] * decay
                interval_end += strain_step
            prev = new
            max_strain = max(new.strains[p_type],max_strain)
            #print max_strain
        difficulty = 0
        weight = 1.0
